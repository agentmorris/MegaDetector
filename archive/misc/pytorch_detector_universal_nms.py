...
class PTDetector:
    ...
    def _process_batch_group(self, group_items, results, detection_threshold, augment, verbose):
        ...
        use_new_nms_function = True

        if use_new_nms_function:
    ...
    def nms(self, prediction, conf_thres=0.25, iou_thres=0.45, max_det=300):
        ...
        Args:
            prediction (torch.Tensor): Model predictions. Format depends on model type:
                - YOLOv5: [batch_size, num_anchors, num_classes + 5]
                - Ultralytics: [batch_size, num_classes + 5, num_anchors]
                Channels: [x_center, y_center, width, height, objectness, class1_conf, class2_conf, ...]
        ...
        # Detect tensor format and transpose if needed
        global yolo_model_type_imported

        # Ultralytics models use [batch, channels, anchors] format and need transposing
        # YOLOv5 models use [batch, anchors, channels] format and work directly
        # This matches what ultralytics non_max_suppression does: prediction.transpose(-1, -2)
        if yolo_model_type_imported == 'ultralytics':
        ...
        for img_idx in range(batch_size):
            ...
            # Follow Ultralytics filtering logic exactly:
            # xc = prediction[:, 4:mi].amax(1) > conf_thres  # candidates
            if yolo_model_type_imported == 'ultralytics':
                # Ultralytics uses prediction[:, 4:mi] where mi = 4 + nc
                # Calculate nc from tensor shape: nc = total_columns - 4 (coordinates)
                total_columns = x.shape[1]
                nc = total_columns - 4  # number of classes
                ...
                # YOLOv5 approach: filter by objectness confidence
                obj_conf = x[:, 4]
            ...
            # Handle coordinate conversion - BOTH model types need conversion
            # Both YOLOv5 and Ultralytics models output [center_x, center_y, width, height] format
            # Convert from [center_x, center_y, width, height] to [x1, y1, x2, y2] format
            ...
            # Handle class confidence calculation differently for different model types
            if yolo_model_type_imported == 'ultralytics':
                # Follow Ultralytics ops.py exactly: box, cls, mask = x.split((4, nc, extra), 1)
                # nc = number of classes = total_columns - 4 (coordinates)
                ...
                # YOLOv5 NMS multiplies class probabilities by objectness confidence
